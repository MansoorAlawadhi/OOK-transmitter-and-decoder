...
#audio file left crop must be alligned with the start of the first preamble bit in order to work
...
abs_data = np.abs(float_data)#absolute the data
...
BinaryData = BinaryData[preamble_size:-postamble_size]#remove the preamble and the postamble so the only data left is the image data(slicing)
